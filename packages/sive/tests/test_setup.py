...
def test_setup_skip_keychain_still_patches_mise_config(capsys):
    ...
    with (
        patch(
            "sive.commands.setup.ensure_bw_cli", return_value=True
        ) as mock_ensure_bw_cli,
        ...
        patch(
            "sive.core.ui.spin", side_effect=lambda _title, fn: fn()
        ),  # transparent passthrough
        # Vault is pre-configured (load_vault succeeds) so the server URL is
...
def test_run_login_passes_own_session_key_to_refresh():
    ...
    mock_run_refresh.assert_called_once_with(
        vault_name="personal", session_key="the-session-key"
    )


def test_setup_idempotent_when_already_unlocked_and_password_stored(capsys):
    """Repeat setup with bw unlocked and password stored.

    Zero prompts, just mise config.
    """
    from sive.core.vaults import VaultConfig
...
def test_patch_mise_config_creates_only_sive_mise_hook_without_bw_tool(tmp_path):
    ...
    with (
        ...
        patch.object(
            setup, "_materialize_mise_hook_script", return_value=setup.SIVE_HOOK_SCRIPT
        ),
    ):
...
def test_patch_mise_config_skips_directive_when_hook_script_missing(tmp_path, capsys):
    """The `_.source` directive is unsafe without the script it points at.

    It must never be written in that case.
    """
    config_dir = tmp_path / "mise"
    config_file = config_dir / "config.toml"
...
def test_patch_mise_config_migrates_legacy_vfox_directive(tmp_path):
    """A pre-existing `_.sive = {}` vfox directive crashes mise, so it is
    replaced."""
    config_dir = tmp_path / "mise"
    config_dir.mkdir()
    config_file = config_dir / "config.toml"
...
def test_mise_hook_noops_when_sive_binary_is_missing():
    hook = PACKAGE_ROOT / "src" / "sive" / "mise_hook" / "env.sh"
    content = hook.read_text(encoding="utf-8")

    assert "command -v sive >/dev/null 2>&1 || return 0" in content
    assert content.index("command -v sive >/dev/null") < content.index(
        "$(sive _mise-env"
    )
