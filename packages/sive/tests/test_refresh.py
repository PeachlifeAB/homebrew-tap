...
def test_refresh_continues_after_invalid_source(capsys):
    with (
        ...
        exit_code = run(
            vault_name="personal", sources=["global", "personal.folder:env/test"]
        )
...
def test_refresh_reuses_caller_session_key_without_reunlocking(tmp_path):
    ...
    vault = VaultConfig(
        name="personal", server="https://vw.example.com", appdata_dir=tmp_path
    )
    ...
    mock_ensure.assert_called_once_with(
        "personal", "caller-session", appdata_dir=str(tmp_path)
    )
