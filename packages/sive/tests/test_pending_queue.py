...
def test_drain_calls_upsert_for_each_entry(state_dir: Path) -> None:
    ...
    with (
        ...
        patch(
            "sive.core.pending_queue.find_folder_id",
            side_effect=["folder-global", "folder-work"],
        ),
...
def test_drain_leaves_queue_on_network_error(state_dir: Path) -> None:
    ...
    # bw.py funnels timeouts, non-zero exits and parse failures into BWError,
    # so that is what a network failure actually reaches this code as.
    with patch(
        "sive.core.pending_queue.list_folders",
        side_effect=BWError("502 Bad Gateway"),
    ):
        drained = drain_pending("personal", session="sess", appdata_dir="/tmp/bw")
