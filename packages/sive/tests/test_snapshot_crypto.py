...
def test_wrong_key_raises():
    ...
    # InvalidTag specifically: AES-GCM authentication failing is what proves
    # the wrong key was rejected, rather than any error at all.
...
def test_ensure_key_creates_when_missing():
    ...
    with (
        patch(
            "sive.core.snapshot_crypto.get_secret",
            side_effect=KeychainError("not found"),
        ),
...
def test_ensure_key_idempotent_across_multiple_calls():
    ...
    def get_side_effect(_vault, _account, **_kwargs):
        nonlocal call_count
        call_count += 1
        if call_count == 1:
