...
def test_load_source_happy_path(
    ...
    mock_list_items.return_value = [
        {"type": 2, "name": "ANTHROPIC_API_KEY", "notes": "sk-xxx"}
    ]
    ...
    mock_unlock.assert_called_once_with(
        "masterpass", appdata_dir=str(Path("/tmp/sive-personal"))
    )
...
def test_load_sources_uses_vault_specific_session_keys(mock_build_context):
    ...
    mock_build_context.side_effect = [
        VaultContext(
            session="session-a",
            appdata_dir="/tmp/a",
            folder_ids={"env/global": "folder-a"},
        ),
        VaultContext(
            session="session-b",
            appdata_dir="/tmp/b",
            folder_ids={"env/global": "folder-b"},
        ),
    ]
