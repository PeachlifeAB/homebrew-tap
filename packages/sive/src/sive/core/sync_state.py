...
# A vault name becomes a path segment, so anything that could escape the
# state directory is rejected.
PATH_TRAVERSAL_MARKERS = ("..", "/", "\\")


def _validate_vault_name(vault_name: str) -> None:
    if not vault_name or any(m in vault_name for m in PATH_TRAVERSAL_MARKERS):
...
def load_known_tags(vault_name: str) -> list[str]:
    """Return cached env/ tag names from the last successful sync.
...
def run_sync_vault(vault_name: str) -> int:
    ...
    try:
        try:
            ...
            _update_known_tags(session, str(vault.appdata_dir), state)
            ...
            # Whatever a sync raises is recorded for `sive status` to show, so
            # this deliberately catches everything below it.
...
def _update_known_tags(session: str, appdata_dir: str, state: dict) -> None:
    """Persist env/ tag names so setup can read them without a bw call."""
    import logging

    from .bw import BWError, list_env_tags

    try:
        state["known_tags"] = list_env_tags(session, appdata_dir=appdata_dir)
    except (BWError, OSError) as e:
...
def _write_snapshot_from_session(vault_name: str, session: str) -> None:
    """Re-read all sources, writing one snapshot per tag on an existing session."""
