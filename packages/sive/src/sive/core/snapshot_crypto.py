...
# The macOS security tool says this when the item does not exist.
KEYCHAIN_MISSING_MARKER = "not found"


def ensure_key(vault_name: str, tag: str) -> None:
    """Create and store a snapshot key for (vault, tag) if one does not exist.
    ...
    try:
        ...
        if KEYCHAIN_MISSING_MARKER not in str(e).lower():
            raise
