...
# Fingerprints of the bw CLI's own stderr. Matched case-insensitively;
# an upstream rewording breaks the match, so they are declared together.
# Bitwarden's item type for a secure note, the only type sive writes.
SECURE_NOTE_TYPE = 2

NOT_LOGGED_IN_MARKERS = ("not logged in", "you are not logged in")
BAD_PASSWORD_MARKERS = ("mac failed", "invalid master password")


def _bw_env(
    env: dict[str, str] | None, appdata_dir: str | Path | None
) -> dict[str, str]:
    full_env = {**os.environ}
    if env:
        full_env.update(env)
...
def _raise_for_failure(
    ...
    """Translate a non-zero bw exit into the narrowest error that fits."""
    stderr = result.stderr.strip()
    lowered = stderr.lower()
    if any(marker in lowered for marker in NOT_LOGGED_IN_MARKERS):
...
def _run(
    ...
    full_env = _bw_env(env, appdata_dir)

    try:
        result = _run_bw_command(args, full_env, timeout)
    except FileNotFoundError as error:
        # bw may simply not be installed yet; install it once and retry.
        if not ensure_bw_cli():
            raise _bw_not_installed_error() from error
        result = _run_bw_command(args, full_env, timeout)
    except subprocess.TimeoutExpired as error:
        raise BWError(
            f"bw command timed out after {timeout}s: bw {' '.join(args[:2])}"
        ) from error

    if result.returncode != 0:
        _raise_for_failure(args, result)
...
def set_server(
    ...
    *,
    ...
    try:
        current_status = (
            status if status is not None else get_status(appdata_dir=appdata_dir)
        )
...
def list_folders(
    ...
    output = _run(
        ["list", "folders", "--session", session_key], appdata_dir=appdata_dir
    )
    try:
        return json.loads(output)
    except json.JSONDecodeError as _error:
...
def create_folder(
    name: str, session_key: str, *, appdata_dir: str | Path | None = None
) -> str:
    ...
    output = _run(
        ["create", "folder", payload, "--session", session_key], appdata_dir=appdata_dir
    )
    try:
    ...
    *,
    ...
    existing = next(
        (
            i
            for i in items
            if i.get("name") == name and i.get("type") == SECURE_NOTE_TYPE
        ),
        None,
    )
    ...
    if existing:
        ...
        _run(
            ["create", "item", payload, "--session", session_key],
            appdata_dir=appdata_dir,
        )


def delete_item(
    item_id: str, session_key: str, *, appdata_dir: str | Path | None = None
) -> None:
...
def delete_folder(
    ...
    _run(
        ["delete", "folder", folder_id, "--session", session_key],
        appdata_dir=appdata_dir,
    )
