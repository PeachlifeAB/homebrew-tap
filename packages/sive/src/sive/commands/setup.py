"""sive login / sive setup — vault authentication and project configuration."""

from __future__ import annotations

import os
import subprocess
import sys
from pathlib import Path

from ..core import ui
from ..core.bw import (
    BWError,
    ensure_bw_cli,
    get_status,
    list_env_tags,
    set_server,
    unlock,
)
from ..core.keychain_macos import KeychainError, store_email, store_password
from ..core.keychain_macos import get_email as get_stored_email
from ..core.keychain_macos import get_password as get_stored_password
from ..core.project_config import write_project_config
from ..core.sync_state import load_known_tags
from ..core.vaults import (
    CONFIG_DIR,
    VAULTS_TOML,
    ConfigError,
    load_vault,
    write_vault_stub,
)

MISE_CONFIG_DIR = Path.home() / ".config" / "mise"
GLOBAL_MISE_CONFIG = MISE_CONFIG_DIR / "config.toml"

SIVE_HOOK_SCRIPT = Path.home() / ".local" / "share" / "sive" / "mise_hook" / "env.sh"
SIVE_MISE_DIRECTIVE = f'_.source = "{SIVE_HOOK_SCRIPT}"'
ENV_CACHE_OFF = "env_cache = false"
ENV_CACHE_ON = "env_cache = true"

# Keys and markers this code reads out of a mise config.
MISE_CONFIG_FILENAME = "config.toml"
ENV_CACHE_KEY = "env_cache"
MISE_SOURCE_KEY = "_.source"
# The vfox-style directive an older sive wrote; it crashes mise.
LEGACY_SIVE_DIRECTIVE = "_.sive"
SIVE_HOOK_PATH_MARKER = "sive/mise_hook/env.sh"
SIVE_MARKER = "sive"
_DONE = "  Done."

MISE_SETTINGS_BLOCK = f"""[settings]
{ENV_CACHE_OFF}
"""

MISE_ENV_BLOCK = f"""
{MISE_SETTINGS_BLOCK.strip()}

[env]
{SIVE_MISE_DIRECTIVE}
"""


def run() -> int:
    # Called only by tests. Not registered as a public CLI command.
    rc, _, _ = _run_login()
    return rc


def _print_keychain_error(error: KeychainError) -> None:
    for line in str(error).splitlines():
        ui.echo(f"  {line}" if line else "", file=sys.stderr)


def _has_stored_password(vault_name: str) -> bool:
    """Non-throwing probe: is a master password already saved in Keychain?"""
    try:
        get_stored_password(vault_name)
        return True
    except KeychainError:
        return False


_SETUP_COMPLETE = "\nSetup complete. Open a new shell to activate sive."

DEFAULT_VAULT = "personal"
GLOBAL_TAG = "global"
# bw status values that mean an account is already authenticated.
BW_LOCKED = "locked"
BW_UNLOCKED = "unlocked"


def _resolve_server(vault, vault_name: str) -> tuple[object, str, str, dict[str, str]]:
    """Return (vault, server_url, appdata_dir, status), prompting on first run.

    An already-configured vault reuses its server; it is never overwritten
    silently.
    """
    if vault is not None:
        return vault, vault.server, str(vault.appdata_dir), {}

    ui.echo(f"\nConfigure vault '{vault_name}':")
    if not VAULTS_TOML.exists():
        ui.echo(f"Creating {VAULTS_TOML} ...")
        CONFIG_DIR.mkdir(parents=True, exist_ok=True)
    server_url = ""
    while not server_url:
        server_url = ui.input("Server URL", placeholder="https://vw.yourdomain.com")
        if not server_url:
            ui.echo("  Server URL is required.")
    write_vault_stub(vault_name, server_url)
    ui.echo(f"  Written to {VAULTS_TOML}")
    vault = load_vault(vault_name)
    return vault, server_url, str(vault.appdata_dir), {}


def _bw_login(email: str, master_password: str, appdata_dir: str) -> int:
    """Run `bw login` under a spinner and return its exit code."""
    env = {
        **os.environ,
        "SIVE_BW_PASSWORD": master_password,
        "BITWARDENCLI_APPDATA_DIR": appdata_dir,
    }
    return ui.spin(
        "Logging in...",
        lambda: (
            subprocess.run(
                ["bw", "login", email, "--passwordenv", "SIVE_BW_PASSWORD"],
                env=env,
                capture_output=True,
                check=False,
            ).returncode
        ),
    )


def _login_to_bitwarden(
    email: str, master_password: str, appdata_dir: str, status: dict[str, str], vault
) -> bool:
    """Log in unless bw already holds a session. False means give up."""
    bw_status = status.get("status", "unauthenticated")
    bw_email = status.get("userEmail", "")

    if bw_status in (BW_LOCKED, BW_UNLOCKED):
        if bw_email.lower() == email.lower():
            ui.echo(f"  Already logged in as {bw_email}.")
            return True
        ui.echo(
            f"  Already logged in as {bw_email}.\n"
            f"  Run 'BITWARDENCLI_APPDATA_DIR={vault.appdata_dir} bw logout'"
            f" first to switch to {email}.",
            file=sys.stderr,
        )
        return False

    if _bw_login(email, master_password, appdata_dir) != 0:
        ui.echo("  Login failed.", file=sys.stderr)
        return False
    ui.echo("  Logged in.")
    return True


def _store_credentials(
    vault_name: str, email: str, master_password: str
) -> tuple[int, str | None, str | None] | None:
    """Persist credentials for silent unlock.

    Returns None to continue setup, or the caller's return value when a Keychain
    failure ends the flow one way or the other.
    """
    try:
        store_password(vault_name, master_password)
        store_email(vault_name, email)
    except KeychainError as e:
        _print_keychain_error(e)
        ui.echo(
            "  Bitwarden login worked, but silent unlock is disabled until "
            "Keychain is fixed.",
            file=sys.stderr,
        )
        if not ui.confirm("Continue setup without silent unlock?", default=True):
            return 1, None, None
        _patch_mise_config()
        ui.echo(_SETUP_COMPLETE)
        ui.echo("Run 'sive setup' again after fixing Keychain to enable silent unlock.")
        return 0, None, None
    ui.echo("  Credentials stored in Keychain.")
    return None


def _initialise_vault(vault_name: str, session_key: str) -> None:
    """Prepare the snapshot key and take the first refresh."""
    from ..core.snapshot_crypto import ensure_key
    from .refresh import run as run_refresh

    ui.spin(
        "Ensuring snapshot encryption key...",
        lambda: ensure_key(vault_name, GLOBAL_TAG),
    )
    ui.echo("  Snapshot key ready in Keychain.")

    # Reuse this login's session key rather than letting refresh derive its own —
    # each `bw unlock` overwrites the vault's active key material for this appdata
    # dir on disk, silently invalidating any session key issued by a prior unlock.
    refresh_rc = ui.spin(
        "Running initial refresh...",
        lambda: run_refresh(vault_name=vault_name, session_key=session_key),
    )
    if refresh_rc != 0:
        ui.echo(
            "  Warning: initial refresh failed — run 'sive refresh' manually.",
            file=sys.stderr,
        )


def _probe_existing_setup(
    vault_name: str,
) -> tuple[object | None, str, dict[str, str]]:
    """Read the vault config and bw status, tolerating an unconfigured machine.

    load_vault raises ConfigError when vaults.toml has no usable entry. The
    fallbacks keep appdata_dir str-typed so every later call site type-checks.
    """
    try:
        vault = load_vault(vault_name)
    except ConfigError:
        return None, "", {}
    appdata_dir = str(vault.appdata_dir)
    status = ui.spin(
        "Checking vault status...", lambda: _get_status_or_empty(appdata_dir)
    )
    return vault, appdata_dir, status


def _already_set_up(
    vault: object | None, status: dict[str, str], vault_name: str
) -> bool:
    """Vault configured, bw unlocked, password in Keychain: nothing to do.

    This is the "run setup again after it is already set up" case, which skips
    every prompt and only ensures the mise env hook is wired.
    """
    return (
        vault is not None
        and status.get("status") == BW_UNLOCKED
        and _has_stored_password(vault_name)
    )


def _configure_server(
    vault: object | None,
    vault_name: str,
    appdata_dir: str,
    status: dict[str, str],
) -> tuple[str, dict[str, str]] | None:
    """Point bw at the vault's server. None means the caller should fail.

    The URL is prompted for only on first run; an already-configured vault
    reuses its own server rather than overwriting it silently. Setting the
    server is idempotent and only resets status when it actually changed.
    """
    if vault is None:
        vault, server_url, appdata_dir, status = _resolve_server(vault, vault_name)
    else:
        server_url = vault.server

    try:
        server_changed = ui.spin(
            f"Configuring bw to use {server_url} ...",
            lambda: set_server(server_url, status=status, appdata_dir=appdata_dir),
        )
    except BWError as e:
        ui.echo(f"  Error: {e}", file=sys.stderr)
        return None

    return appdata_dir, ({} if server_changed else status)


def _run_login() -> tuple[int, str | None, str | None]:
    """Run vault login flow. Returns (exit_code, session_key, appdata_dir)."""
    ui.style(
        "sive setup",
        bold=True,
        foreground="#FAFAFA",
        background="#7D56F4",
        padding="0 1",
    )
    ui.echo()

    # Step 1: Check bw is installed
    ui.echo("Checking for bw CLI...")
    if not ensure_bw_cli():
        return 1, None, None

    vault_name = DEFAULT_VAULT
    vault, appdata_dir, status = _probe_existing_setup(vault_name)

    if _already_set_up(vault, status, vault_name):
        ui.echo(
            f"\nVault '{vault_name}' is already set up "
            f"(logged in as {status.get('userEmail', '?')})."
        )
        _patch_mise_config()
        ui.echo(_SETUP_COMPLETE)
        return 0, None, appdata_dir

    configured = _configure_server(vault, vault_name, appdata_dir, status)
    if configured is None:
        return 1, None, None
    appdata_dir, status = configured

    # Step 6: Credentials are required to log in or re-lock the vault.
    ui.echo("\nLogging in to Bitwarden...")
    email = ui.input("Email")
    master_password = ui.password("Master password")

    if not _login_to_bitwarden(email, master_password, appdata_dir, status, vault):
        return 1, None, None

    if not ui.confirm("Store master password in macOS Keychain for silent unlock?"):
        ui.echo("  Skipped. Silent unlock will not work without a stored password.")
        _patch_mise_config()
        ui.echo(_SETUP_COMPLETE)
        return 0, None, None

    try:
        session_key = ui.spin(
            "Validating unlock...",
            lambda: unlock(master_password, appdata_dir=appdata_dir),
        )
    except BWError as e:
        ui.echo(f"  Unlock failed: {e}", file=sys.stderr)
        ui.echo("  Master password NOT stored. Re-run 'sive setup' to retry.")
        return 1, None, None

    stored = _store_credentials(vault_name, email, master_password)
    if stored is not None:
        return stored

    _initialise_vault(vault_name, session_key)

    _patch_mise_config()

    ui.echo(_SETUP_COMPLETE)
    return 0, session_key, appdata_dir


def _bootstrap_login() -> tuple[int, tuple[str, str] | None]:
    """Connect this machine to the vault if it is not connected yet."""
    if _bootstrap_ready():
        return 0, None
    ui.echo("Sive needs to connect this Mac to your vault first.\n")
    rc, sk, ad = _run_login()
    if rc != 0:
        return 1, None
    return 0, (sk, ad) if sk and ad else None


def _fetch_available_tags(login_session: tuple[str, str] | None) -> list[str]:
    """Read the vault's env tags, or an empty list if it cannot be reached."""
    try:
        # login_session is None when the user skipped keychain storage during
        # _run_login. _unlock_vault also returns None then, and the free-text
        # prompt is the correct fallback.
        session = login_session or _unlock_vault("personal")
        if not session:
            return []
        sk, ad = session
        return list_env_tags(sk, appdata_dir=ad)
    except Exception as e:  # noqa: BLE001
        # Setup proceeds without the tag list rather than failing outright.
        ui.echo(f"\n  Warning: could not load tags from vault ({e})", file=sys.stderr)
        return []


def _prompt_for_tags(login_session: tuple[str, str] | None) -> list[str] | None:
    """Choose from the vault's tags, falling back to free text. None aborts."""
    available = load_known_tags("personal")
    if not available:
        available = ui.spin(
            "Loading tags from vault...", lambda: _fetch_available_tags(login_session)
        )

    if available:
        tags = ui.choose("Select project tags", available)
        if not tags:
            ui.echo("  No tags selected. Aborted.", file=sys.stderr)
            return None
        return list(tags)

    raw = ui.input("Project tag(s) to load", placeholder="e.g. myproject")
    if not raw:
        ui.echo("  No tags entered. Aborted.", file=sys.stderr)
        return None
    return [t.strip() for t in raw.replace(",", " ").split() if t.strip()]


def _ensure_snapshot_keys(vault_name: str, tags: list[str]) -> None:
    """Best effort: a missing key only degrades later snapshot reads."""
    from ..core.snapshot_crypto import ensure_key

    for tag in tags:
        try:
            ensure_key(vault_name, tag)
        except Exception as e:  # noqa: BLE001
            ui.echo(
                f"  Warning: could not create snapshot key for tag '{tag}': {e}",
                file=sys.stderr,
            )


def run_project_setup(tags: list[str] | None = None, no_global: bool = False) -> int:
    ui.style(
        "sive setup",
        bold=True,
        foreground="#FAFAFA",
        background="#7D56F4",
        padding="0 1",
    )
    ui.echo()

    rc, login_session = _bootstrap_login()
    if rc != 0:
        return 1

    if tags is None:
        tags = _prompt_for_tags(login_session)
        if tags is None:
            return 1

    if not tags:
        ui.echo("  No tags provided.", file=sys.stderr)
        return 1

    _patch_mise_config()

    # Auto-prepend global unless opted out or already present
    if not no_global and GLOBAL_TAG not in tags:
        tags = ["global"] + tags

    vault_name = DEFAULT_VAULT
    _ensure_snapshot_keys(vault_name, tags)

    write_project_config(tags, vault=vault_name)
    tag_list = ", ".join(tags)
    ui.echo(f"\n  This directory is now configured for tags: {tag_list}")
    ui.echo("  Secrets will be available automatically in new shells.")
    return 0


def _unlock_vault(vault_name: str) -> tuple[str, str] | None:
    """Silent unlock. Returns (session_key, appdata_dir) or None on any error."""
    try:
        vault = load_vault(vault_name)
        password = get_stored_password(vault_name)
        session_key = unlock(password, appdata_dir=str(vault.appdata_dir))
        return session_key, str(vault.appdata_dir)
    except KeychainError:
        return None
    except BWError as e:
        ui.echo(f"  Warning: silent unlock failed ({e})", file=sys.stderr)
        return None
    except Exception as e:  # noqa: BLE001
        # BWError is handled above; anything else still only warns.
        ui.echo(f"  Warning: unexpected error during unlock ({e})", file=sys.stderr)
        return None


def _prompt_credentials(vault_name: str, bw_email: str) -> tuple[str, str]:
    """Ask for whatever is not already known: the email, and always the password."""
    email = get_stored_email(vault_name) or bw_email
    if email:
        ui.echo(f"  Email: {email}")
    else:
        email = ui.input("Email")
    return email, ui.password("Master password")


def _authenticate(
    bw_status: str,
    bw_email: str,
    email: str,
    master_password: str,
    appdata_dir: str,
) -> bool:
    """Ensure bw holds an account for this email. False means it reported why not.

    An account already present cannot be switched from here, only unlocked, so
    a mismatched email is an error rather than a fresh login.
    """
    if bw_status in (BW_LOCKED, BW_UNLOCKED):
        if bw_email.lower() != email.lower():
            ui.echo(
                f"sive: already logged in as {bw_email}. Run "
                f"'BITWARDENCLI_APPDATA_DIR={appdata_dir} bw logout' to switch.",
                file=sys.stderr,
            )
            return False
        return True

    if _bw_login(email, master_password, appdata_dir) != 0:
        ui.echo("sive: login failed.", file=sys.stderr)
        return False
    return True


def run_relogin(vault_name: str = "personal") -> tuple[int, str | None, str | None]:
    """Re-authenticate a configured vault.

    Returns (exit_code, session_key, appdata_dir).
    """
    try:
        vault = load_vault(vault_name)
    except ConfigError as e:
        ui.echo(f"sive: {e}", file=sys.stderr)
        return 1, None, None

    appdata_dir = str(vault.appdata_dir)
    status = _get_status_or_empty(appdata_dir)

    try:
        set_server(vault.server, status=status, appdata_dir=appdata_dir)
    except BWError as e:
        ui.echo(f"sive: {e}", file=sys.stderr)
        return 1, None, None

    bw_status = status.get("status", "unauthenticated")
    bw_email = status.get("userEmail", "")

    ui.echo("Vault session expired. Please log in again.")
    email, master_password = _prompt_credentials(vault_name, bw_email)

    if not _authenticate(bw_status, bw_email, email, master_password, appdata_dir):
        return 1, None, None

    try:
        session_key = ui.spin(
            "Unlocking...", lambda: unlock(master_password, appdata_dir=appdata_dir)
        )
    except BWError as e:
        ui.echo(f"sive: unlock failed: {e}", file=sys.stderr)
        return 1, None, None

    try:
        store_password(vault_name, master_password)
        store_email(vault_name, email)
    except KeychainError as e:
        _print_keychain_error(e)
        ui.echo(
            "  Logged in for this command, but silent unlock is still disabled.",
            file=sys.stderr,
        )
        return 0, session_key, appdata_dir

    ui.echo("  Logged in and keychain updated.")
    return 0, session_key, appdata_dir


def _bootstrap_ready() -> bool:
    try:
        vault = load_vault("personal")
        get_stored_password("personal")
        status = _get_status_or_empty(str(vault.appdata_dir))
        return status.get("status") in ("locked", "unlocked")
    except (ConfigError, KeychainError):
        return False


def _global_mise_config_path() -> Path:
    """Return the only mise config path this command is allowed to edit."""
    config_dir = MISE_CONFIG_DIR.expanduser().resolve()
    config_path = GLOBAL_MISE_CONFIG.expanduser().resolve()
    if config_path.parent != config_dir:
        raise RuntimeError(
            f"Refusing to edit mise config outside {config_dir}: {config_path}"
        )
    return config_path


def _read_mise_config(config_path: Path) -> str:
    with config_path.open(encoding="utf-8") as f:
        return f.read()


def _assert_mise_config_path(config_path: Path) -> None:
    if config_path.name != MISE_CONFIG_FILENAME:
        raise RuntimeError(
            f"Refusing to edit unexpected mise config filename: {config_path}"
        )


def _write_mise_config(config_path: Path, content: str) -> None:
    _assert_mise_config_path(config_path)
    with config_path.open("w", encoding="utf-8") as f:
        f.write(content)


def _insert_after_header(content: str, header: str, line: str) -> str:
    """Insert `line` right after `header`, or append a new `header`+`line` block
    if `header` is absent. Never duplicates the header."""
    if header not in content:
        sep = "\n" if content.endswith("\n") else "\n\n"
        return content + f"{sep}{header}\n{line}\n"
    idx = content.index(header) + len(header)
    return content[:idx] + f"\n{line}" + content[idx:]


def _merge_sive_directive(content: str) -> str:
    """Add the sive env hook + env_cache=false to existing [settings]/[env]
    tables without disturbing unrelated keys already in those tables."""
    updated = content
    if ENV_CACHE_KEY not in updated:
        updated = _insert_after_header(updated, "[settings]", ENV_CACHE_OFF)
    else:
        updated = updated.replace(ENV_CACHE_ON, ENV_CACHE_OFF)
    updated = _insert_after_header(updated, "[env]", SIVE_MISE_DIRECTIVE)
    return updated


def _materialize_mise_hook_script() -> Path | None:
    """Copy the bundled mise env hook script to a stable user dir and return it.

    Returns None when the script is not shipped with this install.
    """
    import importlib.resources as ir
    import shutil
    import stat

    src = ir.files("sive").joinpath("mise_hook", "env.sh")
    try:
        with ir.as_file(src) as src_path:
            if not Path(src_path).is_file():
                return None
            SIVE_HOOK_SCRIPT.parent.mkdir(parents=True, exist_ok=True)
            shutil.copyfile(src_path, SIVE_HOOK_SCRIPT)
            SIVE_HOOK_SCRIPT.chmod(SIVE_HOOK_SCRIPT.stat().st_mode | stat.S_IEXEC)
    except (ModuleNotFoundError, OSError, ValueError):
        return None
    return SIVE_HOOK_SCRIPT


def _patch_mise_config() -> None:
    """Add sive env directive to global mise config if not already present."""
    if not ui.ensure_homebrew_command("mise", "mise", "mise"):
        return
    # `_.source` runs the script directly — no plugin/vfox resolution involved,
    # unlike the old `_.sive = {}` vfox module directive it replaces.
    if _materialize_mise_hook_script() is None:
        ui.echo(
            "  Warning: bundled mise hook script not found — skipping env hook setup.",
            file=sys.stderr,
        )
        ui.echo(
            "\nFix the sive install and run 'sive setup' again to enable the env hook."
        )
        return

    config_path = _global_mise_config_path()
    config_path.parent.mkdir(parents=True, exist_ok=True)

    if config_path.exists():
        content = _read_mise_config(config_path)
        if LEGACY_SIVE_DIRECTIVE in content or SIVE_HOOK_PATH_MARKER in content:
            if SIVE_MISE_DIRECTIVE in content:
                normalized = content.replace(ENV_CACHE_ON, ENV_CACHE_OFF)
                normalized = normalized.replace('env_cache_ttl = "15m"\n', "")
                if normalized != content:
                    _write_mise_config(config_path, normalized)
                    ui.echo(f"\nUpdated sive config in {config_path} ...")
                    ui.echo(_DONE)
                    return
                return  # already configured, nothing to do
            import re

            # Replace either the legacy vfox directive (`_.sive = {...}`) or a
            # stale `_.source` pointing at the old hook script path.
            updated = re.sub(r"_\.sive\s*=\s*\{[^\n]*\}", SIVE_MISE_DIRECTIVE, content)
            updated = re.sub(
                r'_\.source\s*=\s*"[^"\n]*sive/mise_hook/env\.sh"',
                SIVE_MISE_DIRECTIVE,
                updated,
            )
            updated = updated.replace(ENV_CACHE_ON, ENV_CACHE_OFF)
            updated = updated.replace('env_cache_ttl = "15m"\n', "")
            _write_mise_config(config_path, updated)
            ui.echo(f"\nUpdated sive config in {config_path} ...")
            ui.echo(_DONE)
            return
        # No sive directive yet. Merge into existing [env]/[settings] tables
        # instead of refusing — TOML allows adding keys anywhere within a
        # table's span, up to the next [section] header.
        _write_mise_config(config_path, _merge_sive_directive(content))
        ui.echo(f"\nPatching {config_path} ...")
    else:
        ui.echo(f"\nCreating {config_path} ...")
        _write_mise_config(config_path, MISE_ENV_BLOCK.strip() + "\n")

    ui.echo(_DONE)


def _get_status_or_empty(appdata_dir: str) -> dict[str, str]:
    try:
        return get_status(appdata_dir=appdata_dir)
    except BWError:
        return {}
