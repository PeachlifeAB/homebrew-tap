...
# Process substitution intentionally mirrors stdout to the debug log.
...
testSyncVersion() {
  ...
  if ! command -v "${BGTAIL}" >/dev/null 2>&1
  then
  ...
  if [[ "${version_out}" != ${expected_prefix}*".dev+d"${expected_date} ]]
  then
...
testStartAndReconnect() {
  ...
  if ! printf "%s" "${reconn}" | grep -q "^DONE$"
  then
